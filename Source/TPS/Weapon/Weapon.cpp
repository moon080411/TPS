...
void AWeapon::FireWithProjectile(TWeakObjectPtr<class ATPSCharacter> OwnerCharacter)
{
	...
	FTransform FireTransform = MeshComponent->GetSocketTransform(TEXT("FireSocket"));
	FVector Direction = FireTransform.GetRotation().GetForwardVector() * TraceDistance;

	FActorSpawnParameters SpawnParametor;
	SpawnParametor.Owner = Character;
	ABullet* SpawnBullet = GetWorld()->SpawnActor<ABullet>(ABullet::StaticClass(), FireTransform, SpawnParametor);
...
void AWeapon::FireWithLineTrace(TWeakObjectPtr<class ATPSCharacter> OwnerCharacter)
{
	...
	const FVector Start = MeshComponent->GetSocketLocation(TEXT("FireSocket"));
	...
	bool HitDetected = GetWorld()->LineTraceSingleByChannel(
		HitResult,
		Start,
		End,
